...
import com.sarchami.scala.oop.files.{ DirEntry, Directory }
...
class Cd(dir: String) extends Command {
  override def apply(state: State): State = {
    ...
    // 2. find absoloute path of the directory we want to cd to
    val absoloutePath =
    ...
    val destinationDirectory = doFindEntry(root, absoloutePath)
  ...
  def doFindEntry(root: Directory, path: String): DirEntry = {
    ...
    // 2. navigate to the correcty entry
